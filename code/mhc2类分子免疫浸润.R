...
geneSet <- data.frame('V1' = c("HLA-DMA","HLA-DMB","HLA-DPA1","HLA-DPB1","HLA-DQA1","HLA-DQB1","HLA-DRA","HLA-DRB1","HLA-DOA","HLA-DOB","CD74","CIITA"),
                      'V2' = c("HLA-DMA","HLA-DMB","HLA-DPA1","HLA-DPB1","HLA-DQA1","HLA-DQB1","HLA-DRA","HLA-DRB1","HLA-DOA","HLA-DOB","CD74","CIITA"))
class(geneSet)
geneSet <- geneSet %>%
...
# l <- list()
l <- as.list(a)
l <- setNames(as.list(colnames(a)),  colnames(a))
...
save(ImmSet,ImmSet3,Immune2,file = 'ssGSEA_output1.Rdata')

load(file = 'ssGSEA_output1.Rdata')
# 小提琴图----------
library(ggplot2)
library(tidyverse)
library(ggpubr)
...
ggplot(ImmSet3,aes(x = reorder(`Immune Cell Type`,-`Estimating Score`) , y = `Estimating Score`, fill = RiskGroup)) +
  # # 小提琴图层
  # geom_violin(position = position_dodge(0.9),alpha = 1.2,show.legend = T,#(图例显示参数)
  #             width = 1.8,trim = T,
  #             color = NA) +
  # 箱线图图层
  geom_boxplot(width = 0.35,show.legend = T,
  ...
  labs(x = "MHC class II Type", y = 'ssGSEA Estimating Score') +
  ...
  #                   name = '') +
  scale_fill_manual(values = c("#FF0000",'green'))+ 
  # 添加显著性标记
...
ggsave(filename = './MHCII-ssGSEA.pdf',width = 16,height = 10)
